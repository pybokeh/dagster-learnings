...
@solid(
    ...
    input_defs=[
        InputDefinition(
            ...
            dagster_type=String,
            default_value="https://raw.githubusercontent.com/dagster-io/dagster/master/examples/docs_snippets/docs_snippets/intro_tutorial/cereal.csv"
        )
    ],
    output_defs=[
        OutputDefinition(
...
def fetch_csv(context, url: str) -> DataFrame:
    df = pd.read_csv(url)
...
def transform_name(context, df: DataFrame) -> DataFrame:
...
if __name__ == "__main__":
    ...
    df = results.result_for_solid("transform_name").output_value("dataframe")
